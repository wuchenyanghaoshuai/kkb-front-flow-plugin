...
class QADeployStage extends PullScriptStage {
    ...
    def run() {
        ...
        this.script.node('jenkins-slave-k8s') {
            ...
            serverEnv.add("K8S_DEPLOY_YML=" + "${sub_dir}" + "/release/test/k8s/deploy.yml")
            serverEnv.add("K8S_SVC_YML=" + "${sub_dir}" + "/release/test/k8s/svc.yml")
            serverEnv.add("K8S_DEPLOY_NAME=" + "${deploy_name}")
            serverEnv.add("DOCKER_TAG=" + "test-${version}")
            ...
            this.script.withEnv(serverEnv) {
                ...
                this.script.sh "${deploy_script_dir}/test/deploy.sh"
            }
        }
    }
