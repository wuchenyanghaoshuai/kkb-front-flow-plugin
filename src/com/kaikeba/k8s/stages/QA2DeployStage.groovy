...
class QA2DeployStage extends PullScriptStage {
    ...
    def run() {
        ...
        this.script.node('jenkins-slave-k8s') {
            ...
            serverEnv.add("K8S_DEPLOY_YML=" + "${sub_dir}" + "/release/test2/k8s/deploy.yml")
            serverEnv.add("K8S_SVC_YML=" + "${sub_dir}" + "/release/test2/k8s/svc.yml")
            serverEnv.add("K8S_DEPLOY_NAME=" + "${deploy_name}")
            serverEnv.add("DOCKER_TAG=" + "test2-${version}")
            ...
            this.script.withEnv(serverEnv) {
                ...
                this.script.sh "${deploy_script_dir}/test2/deploy.sh"
            }
        }
    }
