...
class AutoMergeStage extends Stage {
    ...
    AutoMergeStage(project, stageName) {
        ...
        this.branch_origin = project.params.gitlabSourceBranch
    }
    ...
    def merge() {
        ...
        if (this.script.env.DEPLOY_RESULT == "${buildNo}.undo"){
            ...
            this.script.node('jenkins-slave-hc41') {
                this.script.echo "login to jenkins-slave-hc41"
                ...
                this.script.sshagent(['ykqin-devops']) {
                    ...
                    } else if (branch.startsWith('release/') || branch.startsWith('fix/') ) {
                        def tag = branch.replace('release/','').replace('fix/','')
